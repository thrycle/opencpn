...
bool ChartCanvas::MouseEventDrawCircle( wxMouseEvent& event )
{
    // For now just bail out completely if the point clicked is not on the chart
    if(std::isnan(m_cursor_lat))
        return false;
...
void ChartCanvas::StartDropMark(void)
{
    ...
    SetCanvasToolbarItemState( ID_TRACK, true );
    gFrame->SetMasterToolbarItemState( ID_MENU_ADD_POINT_NEW, true );

    HideGlobalToolbar();

}
void ChartCanvas::FinishDropMark(void)
{
    m_markState=0;
    SetCanvasToolbarItemState( ID_TRACK, false ); // ID_TRACK
    gFrame->SetMasterToolbarItemState( ID_MENU_ADD_POINT_NEW, false );
    SetCursor( *pCursorArrow );
