                             ...
                             )
{
    ...
    for( pugi::xml_node child = wpt_node.first_child(); child != 0; child = child.next_sibling() ) {
        ...
        if( !strcmp( pcn, "depth") ) {
            point_depth = child.first_attribute().as_double();
        }
                      ...
                      int layer_id, bool b_change )
{
    ...
    if( Name == _T ( "rte" ) ) {
        ...
        for( pugi::xml_node tschild = wpt_node.first_child(); tschild; tschild = tschild.next_sibling() ) {
            wxString ChildName = wxString::FromUTF8( tschild.name() );
            if( ChildName == _T("routeType") ) {
                wxString type_name = wxString::FromUTF8( tschild.first_child().value());
                pTentRoute->m_RouteTypeString = type_name;
            }
...
static bool GPXCreateWpt( pugi::xml_node node, RoutePoint *pr, unsigned int flags )
{
    ...
    if ( (pr->GetPointDepth() > 0.01 && (flags & OUT_Depth))) {
        double depth = pr->GetPointDepth();
        child = node.append_child("depth");
        child.append_attribute("meter").set_value(depth);
    }
    else
    {
        double depth = 0.0;
        child = node.append_child("depth");
        child.append_attribute("unknow").set_value(depth);
    }
...
static bool GPXCreateRoute( pugi::xml_node node, Route *pRoute )
{
    ...
    if( pRoute->m_RouteTypeString.Len() ) {
        wxCharBuffer buffer=pRoute->m_RouteTypeString.ToUTF8();
        if(buffer.data()) {
            child = node.append_child("routeType");
            child.append_child(pugi::node_pcdata).set_value(buffer.data());
        }
    }
...
static void UpdateRouteA( Route *pTentRoute )
 {
    ...
    while( node ) {
		...
		if( ex_rp ) {
			...
			ex_rp->m_point_depth = prp->m_point_depth;
...
bool NavObjectCollection1::AddGPXWaypoint(RoutePoint *pWP )
{
    ...
    if (pWP->m_point_depth!=0.0)
        std::cout << "waypoint_depth:" << pWP->m_point_depth << std::endl;
    return true;
}
