...
RoutePropDlg::RoutePropDlg( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style ) : wxFrame( parent, id, title, pos, size, style )
{
    ...
    wxBoxSizer* bSizerFirstLine;
    bSizerFirstLine = new wxBoxSizer(wxHORIZONTAL);
	...
	m_stName->Wrap( -1 );
	bSizerFirstLine->Add( m_stName, 0, wxALIGN_CENTER_VERTICAL|wxALL, 5 );

	m_tcName = new wxTextCtrl( m_pnlBasic, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, 0 );
	bSizerFirstLine->Add( m_tcName, 1, wxALL, 5 );

    //bSizerFirstLine->Add( bSizerFirstLine, 0, wxEXPAND, 5 );

	wxBoxSizer* bSizerType;
	bSizerType = new wxBoxSizer( wxHORIZONTAL );
	m_stType = new wxStaticText( m_pnlBasic, wxID_ANY, _("Type"), wxDefaultPosition, wxDefaultSize, 0 );
	m_stType->Wrap( -1 );
	bSizerFirstLine->Add( m_stType, 0, wxALIGN_CENTER_VERTICAL|wxALL, 5 );

    wxString m_choiceTypesChoices[] = { _("Unset"), _("anxian"), _("hangdao_bianjie_1"), _("hangdao_bianjie_2"), _("jinghaoqu") };
	int m_choiceTypesNChoices = sizeof( m_choiceTypesChoices ) / sizeof( wxString );
	m_choiceTypes = new wxChoice( m_pnlBasic, wxID_ANY, wxDefaultPosition, wxDefaultSize, m_choiceTypesNChoices, m_choiceTypesChoices, 0 );
	m_choiceTypes->SetSelection( 0 );
	m_choiceTypes->SetMaxSize( wxSize( 100,-1 ) );

	bSizerFirstLine->Add( m_choiceTypes, 0, wxALL, 5 );

    //bSizerFirstLine->Add( bSizerType, 0, wxALL, 5 );

	bSizerData->Add( bSizerFirstLine, 0, wxEXPAND, 5 );
...
//	//editor by shadow3zz
	m_dataViewListColumnDepth = m_dvlcWaypoints->AppendTextColumn( _("Depth"), wxDATAVIEW_CELL_INERT, 90, static_cast<wxAlignment>(wxALIGN_LEFT), wxDATAVIEW_COL_RESIZABLE );
	m_dataViewListColumnDepth->GetRenderer()->EnableEllipsize( wxELLIPSIZE_END );
