...
//////////////////////////////////////////////////BASIC/////////////////////////////////////////////////
    ...
    //editor by shadow3zz
    gbSizerInnerProperties->Add( 0, 0, 1, wxEXPAND, 5 );
    m_staticTextDepth = new wxStaticText(m_PanelBasicProperties, wxID_ANY, _("Point Depth"));
    gbSizerInnerProperties->Add(m_staticTextDepth, 0, wxALIGN_CENTRE_VERTICAL, 0);
    m_textDepth = new wxTextCtrl(m_PanelBasicProperties, wxID_ANY);
    gbSizerInnerProperties->Add(m_textDepth, 0, wxALL|wxEXPAND, 5);
...
void MarkInfoDlg::SetRoutePoint( RoutePoint *pRP )
{
    ...
    if( m_pRoutePoint ) {
        ...
        m_depth_save = m_pRoutePoint->m_point_depth;
...
void MarkInfoDlg::OnMarkInfoCancelClick( wxCommandEvent& event )
{
    if( m_pRoutePoint ) {
        ...
        m_pRoutePoint->SetPointDepth(m_depth_save);
...
bool MarkInfoDlg::UpdateProperties( bool positionOnly )
{
    if (m_pRoutePoint) {
        ...
        if( m_pRoutePoint->GetPointDepth() > .01 ) {
            m_textDepth->SetValue(wxString::Format("%.1f", m_pRoutePoint->GetPointDepth()));
        } else {
            m_textDepth->SetValue(wxEmptyString);
        }
        m_depth_save = m_pRoutePoint->m_point_depth;
...
bool MarkInfoDlg::SaveChanges()
{
    if( m_pRoutePoint ) {
        ...
        // editor by shadow3zz
        if( m_textDepth->GetValue() == wxEmptyString ) {
            m_pRoutePoint->SetPointDepth(0.0);
        } else {
            double point_depth;
            if( m_textDepth->GetValue().ToDouble(&point_depth) ) {
                m_pRoutePoint->SetPointDepth(point_depth);
            }
        }
        wxString *icon_name = pWayPointMan->GetIconKey( m_bcomboBoxIcon->GetSelection() );
        if(icon_name && icon_name->Length())
