...
void RoutePropDlgImpl::UpdatePoints()
{
    ...
    while( pnode ) {
        pnode->GetData()->SetIconName("Symbol-Circle");
        std::cout << pnode->GetData()->GetIconName() << std::endl;
        ...
        double point_depth = pnode->GetData()->GetPointDepth();
        if( point_depth < .1 ) {
            point_depth = 0.0;
        }
        ...
        data.push_back( wxVariant(wxString::FromDouble( point_depth ))); // Empty column to fill the remaining space (Usually gets squeezed to zero, even if not empty)
...
void RoutePropDlgImpl::SetRouteAndUpdate( Route *pR, bool only_points )
{
    ...
    if ( !only_points )
    {
        ...
        if( m_pRoute->m_RouteTypeString == RTE_TYPE_0){
            m_choiceTypes->SetSelection(1);
            m_pRoute->m_Colour = ::GpxxColorNames[4];
        }
        else if( m_pRoute->m_RouteTypeString == RTE_TYPE_1)
        {
            m_choiceTypes->SetSelection(2);
            m_pRoute->m_Colour = ::GpxxColorNames[1];
            m_pRoute->m_style = (wxPenStyle)::StyleValues[2];
        }
        else if( m_pRoute->m_RouteTypeString == RTE_TYPE_2) {
            m_choiceTypes->SetSelection(3);
            m_pRoute->m_Colour = ::GpxxColorNames[2];
            m_pRoute->m_style = (wxPenStyle)::StyleValues[2];
        }
        else if( m_pRoute->m_RouteTypeString == RTE_TYPE_3) {
            m_choiceTypes->SetSelection(4);
            m_pRoute->m_Colour = ::GpxxColorNames[3];
            m_pRoute->m_style = (wxPenStyle)::StyleValues[2];
        }
        else m_choiceTypes->SetSelection(0);

        //m_tcType->SetValue( m_pRoute->m_RouteTypeString );
...
void RoutePropDlgImpl::SaveChanges()
{
    if( m_pRoute && !m_pRoute->m_bIsInLayer ) {
        ...
        m_pRoute->m_RouteTypeString = m_choiceTypes->GetString(m_choiceTypes->GetSelection());
